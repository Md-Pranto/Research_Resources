# ------------------------------------------------------------------------------
# Fairness-Regulated Dynamic Round Robin (FRD-RR) - Simple Version
...
# Main FRD-RR simulation
print("=== Fairness-Regulated Dynamic Round Robin (FRD-RR) ===")
...
while completed < n:
    ...
    elastic_tq = tq_base * (1 + (1 - j_current) * beta * 1.5)   # scaling factor 1.5
...
# New: Final Fairness Index (JFI) for total scheduling
final_jfi = calculate_jains_fairness_index(acc_cpu, acc_wait, [p[0] for p in processes])
print("\nFinal Fairness Index (JFI):", final_jfi)

print("\nNote: You can compare this with your standard RR code using same input.")
print("FRD-RR should give lower AWT when there are long and short processes.")