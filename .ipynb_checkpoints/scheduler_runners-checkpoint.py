...
def run_MMMRR(processes):
    ...
    jfi = _jfi(acc_cpu, acc_wait, [p[0] for p in processes])
    return {"AWT": total_wait / n, "ATT": total_tat / n, "CS": context_switches, "JFI": jfi}
...
def run_FRD_RR(processes):
    ...
    while completed < n:
        ...
        elastic_tq = min(tq_base * (1 + (1 - j_current) * beta * 1.5), remaining[name])
        time += elastic_tq
        acc_cpu[name] += elastic_tq
        remaining[name] -= elastic_tq
        last_end[name] = time
...
def run_ElvHLVQRR(processes):
    ...
    acc_wait = {p[0]: 0 for p in processes}
    last_end = {p[0]: p[1] for p in processes}
    time, queue, completed, index, context_switches = 0, deque(), 0, 0, 0
    ...
    while completed < n:
        ...
        name = current[0]
        acc_wait[name] += time - last_end[name]
        rbt_list = [remaining[p[0]] for p in list(queue)] + [remaining[name]]
        avg_bt = _mean(rbt_list)
        min_bt = min(rbt_list)
        tq = max(1, math.ceil(math.sqrt(avg_bt * min_bt)))
        tq = min(tq, remaining[name])
        time += tq
        remaining[name] -= tq
        acc_cpu[name] += tq
        last_end[name] = time
